...
class rf(models.Model): #3
    dl_earfcn = models.CharField(max_length=50) # 3350 EARFCN code for DL (only valid if a single cell is configured in rr.conf)
    tx_gain = models.CharField(max_length=50) # 80 Transmit gain (dB).
    rx_gain = models.CharField(max_length=50) # 40
    # Optional parameters:
    dl_freq = models.CharField(max_length=50) # Override DL frequency corresponding to dl_earfcn
    ul_freq = models.CharField(max_length=50) # Override UL frequency corresponding to dl_earfcn (must be set if dl_freq is set)
    device_name = models.CharField(max_length=50) # Device driver family Supported options: "auto" (uses first driver found), "UHD", "bladeRF", "soapy", "zmq" or "Sidekiq"
    device_args = models.CharField(max_length=50) # Arguments for the device driver. Options are "auto" or any string.
                                                  # Default for UHD: "recv_frame_size=9232,send_frame_size=9232"
                                                  # Default for bladeRF: ""
    time_adv_nsamples= models.CharField(max_length=50) #Transmission time advance (in number of samples) to compensate for RF delay
...
"""
class pcap(models.Model):#4
    enable = models.BooleanField() #Enable MAC layer packet captures (true/false)
    filename = models.CharField(max_length=50) #/tmp/enb_mac.pcap File path to use for LTE MAC packet captures
    nr_filename = models.CharField(max_length=50) #/tmp/enb_mac_nr.pcap File path to use for NR MAC packet captures
    s1ap_enable = models.BooleanField() # Enable or disable the PCAP.
    s1ap_filename = models.CharField(max_length=50) #/tmp/enb_s1ap.pcap File name where to save the PCAP.
    mac_net_enable = models.CharField(max_length=50) #false Enable MAC layer packet captures sent over the network (true/false default: false)
    bind_ip = models.CharField(max_length=50) #0.0.0.0 Bind IP address for MAC network trace (default: "0.0.0.0")
    bind_port = models.CharField(max_length=50) # Bind port for MAC network trace (default: 5687)
    client_ip = models.CharField(max_length=50) #127.0.0.1 Client IP address for MAC network trace (default: "127.0.0.1")
    client_port = models.CharField(max_length=50) # Client IP address for MAC network trace (default: 5847)
...
"""
class log(models.Model):#5
    all_level = models.CharField(max_length=50) # Example: see Logging levels
...
"""
class scheduler(models.Model):#6
    policy = models.CharField(max_length=50) # 'time_pf'
...
class expert(models.Model):#12
    pusch_max_its = models.CharField(max_length=50) # These are half iterations
